...
def add_VGG16_conv5_body(model):
    ...
    model.MaxPool('conv1_2', 'pool1', kernel=2, pad=0, stride=2)#MaxPool 2*2*64，stride=2 所以尺寸减半。
    model.Conv('pool1', 'conv2_1', 64, 128, 3, pad=1, stride=1)#通道数从64变到128
    model.Relu('conv2_1', 'conv2_1')
    model.Conv('conv2_1', 'conv2_2', 128, 128, 3, pad=1, stride=1)#卷积第二次
    model.Relu('conv2_2', 'conv2_2')
    model.MaxPool('conv2_2', 'pool2', kernel=2, pad=0, stride=2)#尺寸减半
    model.StopGradient('pool2', 'pool2')
    model.Conv('pool2', 'conv3_1', 128, 256, 3, pad=1, stride=1)#通道数从128变到256
    ...
    model.MaxPool('conv3_3', 'pool3', kernel=2, pad=0, stride=2)#尺寸减半
    model.Conv('pool3', 'conv4_1', 256, 512, 3, pad=1, stride=1)#通道数从256变到512
    ...
    model.MaxPool('conv4_3', 'pool4', kernel=2, pad=0, stride=2)#尺寸减半
...
def add_VGG16_roi_fc_head(model, blob_in, dim_in, spatial_scale):
    ...
    model.FC('pool5', 'fc6', dim_in * 7 * 7, 4096)#FC 全连接层(输入数据名称，输出数据名称，输入节点*7*7 根据Fast-RCNN,将一个图片再分成7*7，输出节点）
    model.Relu('fc6', 'fc6')
